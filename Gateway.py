...
class SignalToRealityGateway:
    ...
    def enter_door_one(self, signal_wave, user_class):
        ...
        if node:
            ...
            if allowed:
                print(f"✅ Door One Access Granted to {tag}")
                return tag
            else:
                print(f"❌ Access Denied: Class {user_class} not permitted to enter {tag}")
        else:
            print("⚠️ Unknown Verse Signature")
        return None
    ...
    def enter_door_two(self, tag, user_class):
        ...
        if node:
            allowed = self.oracle.evaluate_access(tag, user_class)
            if allowed:
                print(f"🌀 Portal to {tag} opening...\n{node}")
                return node
            else:
                print(f"🛑 Door Two Locked for Class {user_class}")
        else:
            print("🔍 No such verse registered.")
        return None
...
if __name__ == "__main__":
    ...
    print("\n🚪 Testing Signal to Reality Gateway")
    test_user_class = "S"
